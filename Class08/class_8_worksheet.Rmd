...
gapminder %>% 
  group_by(country) %>% 
  ggplot(aes(x=year, y=lifeExp, z=country)) +
  geom_line()
```
...
gapminder %>% 
  group_by(country) %>% 
  ggplot(aes(x=year, y=lifeExp, group=country, colour=country=="Rwanda")) +
  # the above highlighted just rwanda
  geom_line(alpha=0.5) +
  # change the legend to somethign more appropriate
  scale_colour_discrete("Country Name", labels=c("Other", "Rwanda"))
```
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  ...
  # this is the exercise stuff (using scales package)
  # we will use the comma format
  scale_x_log10(labels=scales::comma_format()) +
  # separate into continents
  facet_wrap(~continent)
```

__to have the scales different for each plot, you can use
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  ...
  # we will use the comma format
  scale_x_log10(labels=scales::comma_format()) +
  # separate into continents, and let each use diff scales for x and y
...
gapminder %>% 
  ...
  # this will size the points by pop, and colour by continent
  ggplot(aes(gdpPercap, lifeExp, size=pop, colour=continent)) +
  # this facets by continent, nrow=1 put all in 1 row
...
## NOTE if you put an aesthetic in the ggplot it applies globally, but if you put it in a geom below, it will apply only there

## NOTE: colour is the outside of the plotting character while fill is the inside of the character.  by deafult fill=colour unless specified otherwise(?)
```
